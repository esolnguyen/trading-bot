...
class OutcomePredictor:
    ...
    def should_block(self, market_conditions: dict[str, Any]) -> tuple[bool, float | None]:
    ...
    @staticmethod
    def _conditions_to_row(conditions: dict[str, Any], feature_cols: list[str]) -> list[float]:
        mapping: dict[str, float] = {
            "rsi":       float(conditions.get("rsi", 50)),
            "adx":       float(conditions.get("adx", 20)),
            "atr_pct":   float(conditions.get("atr_percentage", 1.5)),
            "trend":     1.0 if conditions.get("trend_direction") == "BULLISH" else
                        (-1.0 if conditions.get("trend_direction") == "BEARISH" else 0.0),
