...
from src.services.analysis.prompt_templates import build_system_prompt
...
class ContextBuilder:
    ...
    def _build_user_message(
        ...
        *,
        ...
        if self._minimal_context:
            price_lines = ["## Market Snapshot"] + [self._price_only_line(s, snapshots) for s in symbols]
            sections.append((0, "Market Snapshot", "\n".join(price_lines)))

            if position_context:
                sections.append((2, "Current Position", f"## Current Position\n{position_context.strip()}"))
            ...
            if self.settings.model_supports_vision:
                ...
                if chart_block:
                    sections.append((7, "Chart Images", f"## Chart Images\n{chart_block}"))
        else:
            market_lines = ["## Market Snapshot"] + [self._market_line(s, snapshots, analyses) for s in symbols]
            sections.append((0, "Market Snapshot", "\n".join(market_lines)))

            pattern_lines = ["## Patterns Detected"] + [self._pattern_line(s, patterns) for s in symbols]
            ...
            if memory_context:
                sections.append((5, "Trading History", f"## Trading History\n{memory_context.strip()}"))
    ...
    def _price_only_line(
        ...
        parts = [f"{display_symbol}: ${snapshot.price:.2f} ({snapshot.change_24h_pct:.2f}% 24h)"]
    ...
    def _pattern_line(symbol: str, patterns: dict[str, PatternResult]) -> str:
        ...
        display_symbol = symbol.replace("USDT", "").replace("BUSD", "")
        pattern_text = ", ".join(pattern_result.patterns) if pattern_result.patterns else "none"
        return (
