...
def get_order_book(symbol: str, limit: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        def process_orders(orders_array):
            ...
            return [
                {"price": float(order[0]), "quantity": float(order[1])}
        ...
        response_data = {
            ...
            "spread": (
                processed_asks[0]["price"] - processed_bids[0]["price"]
            ...
            "spreadPercent": (
                (
                    (processed_asks[0]["price"] - processed_bids[0]["price"])
                    / processed_bids[0]["price"]
                )
                * 100
                if processed_bids and processed_asks and processed_bids[0]["price"] > 0
                else None
            ),
        }
        ...
        logger.info(
            f"Successfully fetched order book for {normalized_symbol}: {len(processed_bids)} bids, {len(processed_asks)} asks"
        )
        ...
        logger.warning(
            f"Validation error for symbol '{symbol}', limit '{limit}': {error_msg}"
        )
        ...
        return create_error_response(
            "binance_api_error", f"Error fetching order book: {str(e)}"
        )
