...
def get_fee_info(symbol: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Fetching fee information for symbol: {symbol if symbol else 'all symbols'}"
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched fee information for {len(processed_fees)} symbol(s)"
        )

        return create_success_response(data=processed_fees, metadata=metadata)
        ...
        return create_error_response(
            "binance_api_error", f"Error fetching fee information: {str(e)}"
        )
