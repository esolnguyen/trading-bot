...
def get_orders(
    ...
    try:
        ...
        orders = client.get_all_orders(
            symbol=normalized_symbol, start_time=start_time, end_time=end_time
        )

        logger.info("Successfully fetched orders for symbol: %s", symbol)

        response_data = {"symbol": normalized_symbol, "orders": orders}
        ...
        return create_error_response(
            "binance_api_error", f"Error fetching orders: {str(e)}"
        )
