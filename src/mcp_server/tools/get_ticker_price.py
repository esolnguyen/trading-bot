...
def get_ticker_price(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched price for {normalized_symbol}: {response_data['price']}"
        )

        return create_success_response(
            data=response_data,
            metadata={"source": "binance_api", "endpoint": "ticker_price"},
        )
        ...
        return create_error_response(
            "binance_api_error", f"Error fetching ticker price: {str(e)}"
        )
