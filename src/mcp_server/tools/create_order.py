...
def create_order(
    ...
    """
    logger.info(
        f"Creating order: {symbol}, Side: {side}, Type: {order_type}, Quantity: {quantity}"
    )
    ...
    try:
        ...
        validated_quantity = validate_positive_number(
            quantity, "quantity", min_value=0.0
        )
        ...
        if order_type.upper() in [
            ...
            if price is None:
                return create_error_response(
                    "validation_error", f"Price is required for {order_type} orders"
                )
        ...
        return create_error_response(
            "binance_api_error", f"Error creating order: {str(e)}"
        )
