...
def get_ticker_price(symbol: str) -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_ticker_price import (
            get_ticker_price as _get_ticker_price,
        )
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch price for {symbol}: {result.get('error', {}).get('message')}"
            )
...
def get_ticker(symbol: str) -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_ticker import get_ticker as _get_ticker
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch ticker stats for {symbol}: {result.get('error', {}).get('message')}"
            )
...
def get_available_assets() -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_available_assets import (
            get_available_assets as _get_available_assets,
        )
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch available assets: {result.get('error', {}).get('message')}"
            )
...
def get_balance() -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_balance import get_balance as _get_balance
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch account balances: {result.get('error', {}).get('message')}"
            )
...
def get_orders(
    ...
    """
    logger.info(
        f"Tool called: get_orders with symbol={symbol}, start_time={start_time}, end_time={end_time}"
    )

    try:
        from mcp_server.tools.get_orders import get_orders as _get_orders
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch orders for {symbol}: {result.get('error', {}).get('message')}"
            )
...
def get_position_info() -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_position_info import (
            get_position_info as _get_position_info,
        )
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch position info: {result.get('error', {}).get('message')}"
            )
...
def get_pnl() -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_pnl import get_pnl as _get_pnl
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch PnL info: {result.get('error', {}).get('message')}"
            )
...
def create_order(
    ...
    """
    logger.info(
        f"Tool called: create_order with symbol={symbol}, side={side}, type={order_type}, quantity={quantity}, price={price}"
    )

    try:
        from mcp_server.tools.create_order import create_order as _create_order
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to create order for {symbol}: {result.get('error', {}).get('message')}"
            )
...
def get_liquidation_history() -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_liquidation_history import (
            get_liquidation_history as _get_liquidation_history,
        )
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch liquidation history: {result.get('error', {}).get('message')}"
            )
...
def get_deposit_address(coin: str) -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_deposit_address import (
            get_deposit_address as _get_deposit_address,
        )
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch deposit address for {coin}: {result.get('error', {}).get('message')}"
            )
...
def get_deposit_history(coin: str) -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_deposit_history import (
            get_deposit_history as _get_deposit_history,
        )
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch deposit history for {coin}: {result.get('error', {}).get('message')}"
            )
...
def get_withdraw_history(coin: str) -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_withdraw_history import (
            get_withdraw_history as _get_withdraw_history,
        )
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch withdrawal history for {coin}: {result.get('error', {}).get('message')}"
            )
...
def get_account_snapshot(account_type: str = "SPOT") -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_account_snapshot import (
        ...
        if result.get("success"):
            logger.info(
                f"Successfully fetched account snapshot for {account_type} account"
            )
        else:
            logger.warning(
                f"Failed to fetch account snapshot for {account_type}: {result.get('error', {}).get('message')}"
            )
...
def get_fee_info(symbol: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_fee_info import get_fee_info as _get_fee_info
        ...
        if result.get("success"):
            ...
            logger.info(
                f"Successfully fetched fee information for {fee_count} symbol(s)"
            )
        else:
            logger.warning(
                f"Failed to fetch fee information: {result.get('error', {}).get('message')}"
            )
...
def get_order_book(symbol: str, limit: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        from mcp_server.tools.get_order_book import get_order_book as _get_order_book
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to fetch order book for {symbol}: {result.get('error', {}).get('message')}"
            )
...
def validate_configuration() -> bool:
    ...
    try:
        ...
        from mcp_server.security import SecurityConfig, validate_api_credentials
        ...
        security_audit_log(
            "configuration_validation_failed", {"error": str(e)}, level="ERROR"
        )
        return False
...
def main() -> None:
    ...
    try:
        if args.transport == "stdio":
            ...
            logger.info(
                f"Initializing {args.transport} transport on {args.host}:{args.port}"
            )
