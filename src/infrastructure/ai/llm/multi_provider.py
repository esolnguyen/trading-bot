...
def build_orchestrator(settings: Settings) -> Any:
    ...
    clients = ProviderClients(
        google=GoogleAIClient(settings=settings, logger=_logger) if settings.google_studio_api_key else None,
        google_paid=GoogleAIClient(settings=settings, logger=_logger, use_paid=True) if settings.google_studio_paid_api_key else None,
        openrouter=OpenRouterClient(settings=settings, logger=_logger) if settings.openrouter_api_key else None,
        lmstudio=LMStudioClient(settings=settings, logger=_logger),
        blockrun=BlockRunClient(settings=settings, logger=_logger) if settings.blockrun_wallet_key else None,
    )
    return ProviderOrchestrator(settings=settings, logger=_logger, provider_clients=clients)
    ...
    for attempt in range(3):
        try:
            if chart_b64 and settings.model_supports_vision:
                provider_result = await orchestrator.get_chart_response(provider, messages, chart_b64)
            else:
                provider_result = await orchestrator.get_text_response(provider, messages)
