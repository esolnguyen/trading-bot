...
def load_settings_from_env(cls: type["Settings"], env_file: str | None = None) -> "Settings":
    ...
    return cls(
        ...
        bot_mode=bot_mode,
        model_supports_vision=parse_bool(os.getenv("MODEL_SUPPORTS_VISION"), default=False),
        # RAG / ChromaDB
        ...
        google_code_execution=parse_bool(os.getenv("GOOGLE_CODE_EXECUTION"), default=False),
        # Trading parameters
        timeframe=os.getenv("TIMEFRAME", "1h").strip(),
        candle_limit=parse_int(os.getenv("CANDLE_LIMIT"), default=200),
        ai_chart_candle_limit=parse_int(os.getenv("AI_CHART_CANDLE_LIMIT"), default=120),
        ai_chart_timeframe=os.getenv("AI_CHART_TIMEFRAME", "5m").strip(),
        demo_quote_capital=parse_float(os.getenv("DEMO_QUOTE_CAPITAL"), default=10000.0),
        transaction_fee_percent=parse_float(
            os.getenv("TRANSACTION_FEE_PERCENT"), default=0.00075
        ),
        default_stop_loss_pct=parse_float(os.getenv("DEFAULT_STOP_LOSS_PCT"), default=0.02),
        default_take_profit_pct=parse_float(
            os.getenv("DEFAULT_TAKE_PROFIT_PCT"), default=0.04
        ),
        default_position_size=parse_float(os.getenv("DEFAULT_POSITION_SIZE"), default=0.02),
        ...
        file_message_expiry=parse_int(os.getenv("FILE_MESSAGE_EXPIRY_HOURS"), default=168) * 3600,
        ...
        max_consecutive_losses=parse_int(os.getenv("MAX_CONSECUTIVE_LOSSES"), default=3),
        ...
        partial_tp1_size_pct=parse_float(os.getenv("PARTIAL_TP1_SIZE_PCT"), default=0.5),
        # Regime detection
