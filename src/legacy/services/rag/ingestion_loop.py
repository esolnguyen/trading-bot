...
from src.legacy.services.rag.filter import (
    is_relevant_article,
    prepare_news_document,
    sha256_text,
)
from src.legacy.services.rag.sources.alternative_me import AlternativeMeSource
from src.legacy.services.rag.sources.coingecko import CoinGeckoFearGreedSource
from src.legacy.services.rag.sources.cryptocompare import CryptoCompareNewsSource
from src.legacy.services.rag.sources.defillama import DefiLlamaSource
from src.legacy.services.rag.sources.ohlcv_history import OHLCVHistorySource
...
class IngestionLoop:
    ...
    async def run(self) -> None:
        await asyncio.gather(
            self._run_source_forever(
                "news", self.settings.news_interval, self.ingest_news_once
            ),
            self._run_source_forever(
                "coingecko", self.settings.macro_interval, self.ingest_macro_once
            ),
            self._run_source_forever(
                "alternative_me", self.settings.ohlcv_interval, self.ingest_macro_once
            ),
            self._run_source_forever(
                "defillama", self.settings.ohlcv_interval, self.ingest_macro_once
            ),
            self._run_source_forever(
                "ohlcv_history", self.settings.ohlcv_interval, self.ingest_macro_once
            ),
        )
        ...
        try:
            ...
            for document in documents:
                ...
                if not is_relevant_article(
                    prepared.get("title", ""), prepared.get("body", "")
                ):
                    continue
                ...
                if self.sentiment_scorer is not None:
                    text_for_scoring = (
                        f"{prepared.get('title', '')} {prepared.get('body', '')[:400]}"
                    )
            ...
            if status == 429:
                self.logger.warning(
                    "Skipping news cycle after HTTP 429 from %s", source_name
                )
                return
            self.logger.error("HTTP error in news source %s: %s", source_name, exc)
        except requests.ConnectionError as exc:
            self.logger.error(
                "Connection error in news source %s: %s", source_name, exc
            )
        except Exception as exc:  # noqa: BLE001
            self.logger.error(
                "Unexpected error in news source %s: %s", source_name, exc
            )
    ...
    async def ingest_macro_once(self, source_name: str) -> None:
        ...
        try:
            ...
            if status == 429:
                self.logger.warning(
                    "Skipping macro cycle after HTTP 429 from %s", source_name
                )
                return
            self.logger.error("HTTP error in macro source %s: %s", source_name, exc)
        except requests.Timeout as exc:
            self.logger.warning(
                "Timeout fetching macro source %s, will retry next cycle: %s",
                source_name,
                exc,
            )
        except requests.ConnectionError as exc:
            self.logger.warning(
                "Connection error in macro source %s, will retry next cycle: %s",
                source_name,
                exc,
            )
        except Exception as exc:  # noqa: BLE001
            self.logger.error(
                "Unexpected error in macro source %s: %s", source_name, exc
            )

    async def _run_source_forever(
        self, source_name: str, cadence: int, runner: Any
    ) -> None:
        while True:
