...
class MemoryManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._max_entries: int = (
            getattr(settings, "trade_memory_max_entries", 500) if settings else 500
        )
    ...
    def record(self, outcome: TradeOutcome) -> None:
        ...
        document = (
            f"{decision.symbol} {decision.action.value} reasoning: {decision.reasoning}"
        )
        metadata = {
    ...
    def _prune_if_needed(self) -> None:
        try:
            ...
            if ids_to_delete:
                ...
                logger.debug(
                    "trade_memory pruned %d entries (was %d)", len(ids_to_delete), count
                )
