"""Trade execution on Binance UM futures via the SDK.
...
class Executor:
    """Submit validated trade decisions through the Binance UM futures SDK."""
    ...
    async def get_live_position_size(self, symbol: str) -> float:
        ...
        client = await self._ensure_api_client()
        try:
            positions = await asyncio.to_thread(client.get_position_risk, symbol=symbol)
    ...
    async def _apply_leverage(self, client: Any) -> None:
        """Set configured leverage on all trading symbols."""
        leverage = getattr(self.settings, "futures_leverage", 1)
        if leverage == 1:
            return
        for symbol in getattr(self.settings, "trading_symbols", []):
            try:
