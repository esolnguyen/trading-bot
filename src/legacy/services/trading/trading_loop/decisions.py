...
class DecisionsMixin:
    ...
    async def _decide_per_symbol(
        ...
        *,
        ...
        if self.signal_scorer is not None:
            ...
            for symbol in snapshots:
                ...
                if (
                    ...
                    and getattr(self.trading_strategy, "symbol", None) == symbol
                ):
                ...
                if pos_dir is None and symbol in self._open_positions:
                    ...
                    pos_dir = (
                        "LONG"
                        if raw_dir == "BUY"
                        else ("SHORT" if raw_dir == "SELL" else None)
                    )
                ...
                if (
                    ...
                    remaining = cooldown - (
                        self._cycle - self._last_close_cycle[symbol]
                    )
                    self.logger.info(
        ...
        # LLM paths below
        if (
            getattr(self.settings, "single_symbol_decision", False)
            and len(snapshots) > 1
        ):
    ...
    async def _decide_all_symbols_single_call(
        ...
        *,
        ...
        shared_macro = self.retriever.retrieve_macro(
            "global macro crypto market conditions"
        )
        if shared_macro:
    ...
    async def _decide(
        ...
        if decision.action == Action.HOLD:
            return decision
        valid_symbols = set(
            getattr(self.settings, "trading_symbols", ["BTCUSDT", "ETHUSDT"])
        )
        ...
        if (
            ...
            if htf is not None:
                ...
                if decision.action == Action.BUY and htf_signal in (
                    Signal.SELL,
                    Signal.STRONG_SELL,
                ):
                    self.logger.warning(
                        "HTF confirmation failed: 1h=BUY but %s=%s — overriding to HOLD",
                        getattr(self.settings, "htf_timeframe", "4h"),
                        htf_signal.value,
                    )
                    return TradeDecision(
                        symbol=decision.symbol,
                        action=Action.HOLD,
                        reasoning=f"htf_disagreement:{htf_signal.value}",
                        source="htf_filter",
                    )
                if decision.action == Action.SELL and htf_signal in (
                    Signal.BUY,
                    Signal.STRONG_BUY,
                ):
                    self.logger.warning(
