...
class ContextMixin:
    ...
    async def _collect_snapshots(self) -> dict[str, Any]:
        symbols = getattr(self.settings, "trading_symbols", None) or [
            "BTCUSDT",
            "ETHUSDT",
        ]
        ...
        snapshots = await asyncio.gather(
            *[
                self.aggregator.snapshot(s, timeframe=timeframe, limit=limit)
                for s in symbols
            ]
        )
        ...
        for symbol, snapshot in snapshots.items():
            ...
            if self.settings.model_supports_vision:
                ...
                if feed is not None and chart_tf != primary_tf:
                    try:
                        chart_candles = await feed.get_ohlcv(
                            symbol, timeframe=chart_tf, limit=chart_limit
                        )
                    except Exception:  # noqa: BLE001
                        self.logger.debug(
                            "chart candle fetch failed for %s/%s — using primary candles",
                            symbol,
                            chart_tf,
                        )
                pattern_result.chart_png_b64 = self.chart_gen.render(
                    symbol, chart_candles, analyses[symbol].indicators
                )
    ...
    def _build_rag_context_per_symbol(
        ...
        *,
        ...
        for symbol in snapshots:
            ...
            result[symbol] = (
                section
                if section and section != "=== NO CONTEXT AVAILABLE ==="
    ...
    async def _build_balance_context(self, snapshots: dict[str, Any]) -> dict[str, Any]:
        if self.settings.bot_dry_run:
            return {
                "prices": {
                    symbol: snapshot.price for symbol, snapshot in snapshots.items()
                }
            }
    ...
    async def _collect_htf_analyses(
        ...
        for symbol, snapshot in snapshots.items():
            try:
                htf_candles = await feed.get_ohlcv(
                    symbol, timeframe=htf_tf, limit=htf_limit
                )
                if len(htf_candles) < 50:
                    continue
                from src.mcp_servers.shared.domain.market import (
                    MarketSnapshot as _MS,
                )  # noqa: PLC0415
    ...
    async def _build_ml_context(
        ...
        for sym, snap in snapshots.items():
            scorer = (
                self._per_symbol_scorers.get(sym.upper()) or self._percentile_scorer
            )
            if scorer is None:
                continue
            try:
        ...
        if self._multi_tf_analyzer is not None and first_symbol:
            try:
                current_signal = (
                    analyses[first_symbol].signal.value
                    if first_symbol in analyses
                    else "NEUTRAL"
                )
                mtf_text = await self._multi_tf_analyzer.build_summary(
                    first_symbol, current_signal
                )
                if mtf_text:
        ...
        if self._key_level_detector is not None:
            for sym, snap in snapshots.items():
                try:
                    levels_text = self._key_level_detector.format_context(
                    ...
                    self.logger.debug(
                        "key_level_detector failed for %s", sym, exc_info=True
                    )
        ...
        if self._direction_classifier is not None:
            for sym, snap in snapshots.items():
                try:
                    ...
                    self.logger.debug(
                        "direction_classifier failed for %s", sym, exc_info=True
                    )
    ...
    def _refresh_regime(self) -> None:
        ...
        try:
            ...
            features = {
                ...
                "realized_vol": (
                    sum((closes[i] / closes[i - 1] - 1) ** 2 for i in range(-30, 0))
                    / 30
                )
                ** 0.5
                * (365**0.5),
            ...
            if result is not None:
                ...
                suffix = self._cycle_classifier.regime_system_prompt_suffix(
                    regime, confidence
                )
                self.builder.set_regime_suffix(suffix)
                self.logger.info(
                    "Macro regime updated: %s (%.0f%%)", regime, confidence * 100
                )
    ...
    def _attach_pnl(self, outcome: Any, snapshots: dict[str, Any]) -> Any:
        ...
        from src.legacy.domain.trading import TradeOutcome  # noqa: PLC0415
        ...
        exit_price = (
            ...
            or snapshots.get(sym, None)
            and snapshots[sym].price
            or 0.0
        )
        if exit_price <= 0:
            return outcome
