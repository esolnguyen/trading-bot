...
class TradingLoop(
        ...
        try:
            while not self._stop_event.is_set():
                try:
                    ...
                    self.logger.exception("Cycle error — retrying in %ss", _retry_delay)
                ...
                if getattr(self, "_webhook_trigger", False):
                    ...
                    self.logger.info(
                        "Webhook signal received — skipping sleep for immediate cycle"
                    )
                    continue
    ...
    async def run_cycle_once(self) -> dict[str, Any]:
        ...
        if hasattr(self.risk, "check_kill_switches"):
            ...
            if halt:
                self.logger.warning(
                    "KILL SWITCH ACTIVE: %s — skipping cycle %d",
                    halt_reason,
                    self._cycle,
                )
                return {
            ...
            try:
                for snap in snapshots.values():
                    if self._anomaly_detector.is_anomaly(snap, None):
                        self.logger.warning(
                            "Anomaly detected for %s — skipping cycle %d",
                            snap.symbol,
                            self._cycle,
                        )
                        return {
            ...
            for sym, snap in snapshots.items():
                ...
                if writer is not None:
                    try:
                        ...
                        self.logger.debug(
                            "ohlcv_writer.append failed for %s", sym, exc_info=True
                        )
        ...
        if self.trading_strategy is not None:
            try:
                ...
                async with self._position_lock:
                    ...
                    _pre_pnl_pct = (
                        _pre_pos.calculate_pnl(_strat_price) if _pre_pos else None
                    )
                    _pre_quote = _pre_pos.quote_amount if _pre_pos else 0.0

                    close_reason = await self.trading_strategy.check_position(
                        _strat_price
                    )
                    ...
                    if close_reason in (
                        ...
                        self._update_loss_tracking(
                            close_reason, _pre_pnl_pct, _pre_quote
                        )
        ...
        htf_analyses = (
            await self._collect_htf_analyses(
                snapshots,
                choppiness_threshold,
                rsi_strong_buy,
                rsi_buy,
        ...
        if self.signal_scorer is not None:
            ...
            single_call = (
                getattr(self.settings, "single_symbol_decision", True)
                and len(snapshots) > 1
            )
            rag_per_symbol = self._build_rag_context_per_symbol(
                snapshots, analyses, single_call_mode=single_call
            )
        ...
        if self._open_positions:
            ...
            try:
                ...
                self.logger.debug(
                    "trading_strategy.get_position_context failed", exc_info=True
                )
        ...
        if self.memory_service is not None:
            try:
                ...
                self.logger.debug(
                    "memory_service.get_memory_context failed", exc_info=True
                )

        if self.brain_service is not None:
            try:
                first_symbol, first_snap = next(iter(snapshots.items()))
                market_conditions = self.tech_analyzer.get_market_conditions(
                    first_symbol, first_snap
                )
        ...
        for symbol, decision in decisions.items():
            ...
            if (
                decision.action in (Action.CLOSE_LONG, Action.CLOSE_SHORT, Action.CLOSE)
            ...
            outcome = await self.executor.execute(
                risk_result.decision, risk_result.dry_run
            )
            ...
            if (
                ...
                in (Action.CLOSE_LONG, Action.CLOSE_SHORT, Action.CLOSE)
            ):
            ...
            if not outcome.dry_run and outcome.order_id:
                ...
                _exec_price = outcome.executed_price or balance.get("prices", {}).get(
                    symbol, 0.0
                )
                if _exec_price > 0 and isinstance(balance.get("USDT"), dict):
                    _notional = outcome.decision.quantity * _exec_price
                    balance["USDT"]["free"] = max(
                        0.0, balance["USDT"]["free"] - _notional
                    )
        ...
        symbol_signals = [
            (s.replace("USDT", ""), analyses[s].signal.value) for s in analyses
        ]
        ...
        self.console_notifier.notify_cycle(
            ...
            final_decision=(
                summary_outcome.decision.action.value if summary_outcome else "HOLD"
            ),
        )
        decisions_log = " | ".join(
        ...
        dry_run_flag = (
            last_outcome.dry_run if last_outcome else self.settings.bot_dry_run
        )
        ...
        if self.discord_notifier is not None:
            for symbol, (outcome, _) in outcomes.items():
                ...
                try:
                    ...
                    self.logger.debug(
                        "discord_notifier.send_trading_decision failed", exc_info=True
                    )
