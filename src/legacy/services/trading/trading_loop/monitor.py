...
class MonitorMixin:
    ...
    async def _monitor_check_positions(self) -> None:
        ...
        for symbol in symbols:
            try:
                ...
                # --- Strategy-path check (has trailing stop, ATR-based SL/TP) ---
                if self.trading_strategy:
                    async with self._position_lock:
                        strat_pos = getattr(
                            self.trading_strategy, "current_position", None
                        )
                        if strat_pos and strat_pos.symbol == symbol:
                            active_sl = (
                                strat_pos.trailing_stop_price or strat_pos.stop_loss
                            )
                            tp = strat_pos.take_profit
                            is_long = strat_pos.direction == "LONG"
                            sl_hit = (
                                (price <= active_sl)
                                if is_long
                                else (price >= active_sl)
                            )
                            tp_hit = (tp is not None) and (
                                (price >= tp) if is_long else (price <= tp)
                            )
                            if sl_hit or tp_hit:
                                if sl_hit:
                                    reason = (
                                        "trailing_stop"
                                        if strat_pos.trailing_stop_price
                                        else "stop_loss"
                                    )
                                    self.logger.warning(
                                    ...
                                    reason = "take_profit"
                                    self.logger.warning(
                                        "MONITOR [strategy]: TP hit for %s %s @ $%.2f (TP=$%.2f)",
                                        symbol,
                                        strat_pos.direction,
                                        price,
                                        tp,
                ...
                async with self._position_lock:
                    ...
                    tp_price_dict: Optional[float] = pos_dict.get("tp_price")
                    if sl_price_dict is None and tp_price_dict is None:
                        continue
                    direction = pos_dict.get("direction", "BUY")
                    is_long_dict = direction == "BUY"
                    sl_hit = sl_price_dict is not None and (
                        (price <= sl_price_dict)
                        if is_long_dict
                        else (price >= sl_price_dict)
                    )
                    tp_hit = tp_price_dict is not None and (
                        (price >= tp_price_dict)
                        if is_long_dict
                        else (price <= tp_price_dict)
                    )
                    if sl_hit:
                        self.logger.warning(
                        ...
                        self.logger.warning(
                            "MONITOR [executor]: TP hit for %s %s @ $%.2f (TP=$%.2f)",
                            symbol,
                            direction,
                            price,
                            tp_price_dict,
                        )
                        await self._monitor_execute_close(symbol, price, "take_profit")
    ...
    async def _monitor_execute_close(
        ...
        if not self.settings.bot_dry_run and pos_dict:
            ...
            try:
                ...
                self.logger.info(
                    "MONITOR: market close executed for %s, order_id=%s",
                    symbol,
                    outcome.order_id,
                )
                self._check_slippage(
                    outcome, {symbol: type("_S", (), {"price": price})()}
                )
            except Exception:  # noqa: BLE001
                self.logger.exception(
                    "MONITOR: failed to execute market close for %s", symbol
                )
        elif self.settings.bot_dry_run:
            self.logger.info(
                "MONITOR [dry-run]: would close %s @ $%.2f (%s)", symbol, price, reason
            )
        ...
        if symbol in self._open_positions:
            ...
            self.logger.info(
                "MONITOR: cleared position state for %s (%s)", symbol, reason
            )
