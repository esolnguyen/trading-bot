...
class PositionsMixin:
    ...
    async def _update_open_positions(self, outcome: Any) -> None:
        ...
        if existing:
            ...
            is_close = (action == Action.BUY and existing_dir == "SELL") or (
                action == Action.SELL and existing_dir == "BUY"
            )
            if is_close:
        ...
        if entry_price > 0 and action in (Action.BUY, Action.SELL):
            ...
            if strategy_pos and strategy_pos.symbol == sym:
                ...
                sl_price = round(
                    entry_price * ((1 - sl_pct) if is_long else (1 + sl_pct)), 2
                )
                tp_price = round(
                    entry_price * ((1 + tp_pct) if is_long else (1 - tp_pct)), 2
                )
    ...
    def _sync_position_from_strategy(self) -> None:
        ...
        if pos is None:
            if sym in self._open_positions:
                ...
                self._last_close_cycle[sym] = self._cycle
                self.logger.debug(
                    "_sync_position_from_strategy: removed stale dict entry for %s", sym
                )
        else:
