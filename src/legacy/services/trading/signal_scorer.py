...
from src.mcp_servers.shared.domain.analysis import Signal, TechnicalAnalysis
...
class SignalScorer:
        ...
        *,
        ...
        if direction_score is not None:
            ...
            logger.debug(
                ...
                symbol,
                s.scoring_w_direction * 100,
            )

        raw_score = sum(components[k] * weights[k] for k in components)

        comp_str = " ".join(
            f"{k}={v:+.3f}(w={weights[k]:.2f})" for k, v in components.items()
        )
        logger.info(
            "[scorer] %s raw=%.4f %s components: %s",
            symbol,
            raw_score,
        ...
        logger.info(
            ...
            symbol,
            raw_score,
            s.scoring_entry_threshold,
            tags or "none",
            (
                "BUY"
                if raw_score >= s.scoring_entry_threshold
                else ("SELL" if raw_score <= -s.scoring_entry_threshold else "HOLD")
            ),
        )

        # --- Outcome predictor gate ---
        if self._outcome_predictor is not None:
            market_cond = self._build_market_conditions(
                indicators, price, analysis.signal
            )
            ...
            if win_prob is not None and win_prob < self._outcome_predictor.threshold:
                ...
                return self._make_hold(
                    symbol, raw_score, components, tags, f"low_win_prob={win_prob:.0%}"
                )
    ...
    def _score_direction(
        self, indicators: Any, price: float, symbol: str
    ) -> float | None:
    ...
    def _format_reasoning(
        score: float, components: dict[str, float], tags: list[str]
    ) -> str:
    ...
    def _build_market_conditions(
        indicators: Any, price: float, signal: Signal
    ) -> dict[str, Any]:
