...
from src.legacy.domain.trading import Action, RiskValidationResult, TradeDecision

if TYPE_CHECKING:
    from src.legacy.domain.trading.models import RiskAssessment
...
class RiskManager:
    ...
    def validate(
        ...
        if not self.settings.bot_enabled:
            self.logger.warning("Risk rule triggered: BOT_MODE=off; overriding to HOLD")
        ...
        if decision.action in (
            ...
            self.logger.debug(
                "%s passthrough — no sizing/balance checks needed",
        ...
        if effective_price <= 0:
            self.logger.warning(
                "Risk rule triggered: cannot determine price for %s", decision.symbol
            )
            return RiskValidationResult(
        ...
        if decision.quantity <= 0 and decision.action in (Action.BUY, Action.SELL):
            ...
            conf_key = (
                ...
                else ("MEDIUM" if decision.confidence >= 40 else "LOW")
            )
        ...
        if margin > self.settings.max_order_usdt:
            clamped_quantity = (
                self.settings.max_order_usdt * leverage
            ) / effective_price
            self.logger.warning(
