...
from src.legacy.services.llm_trader.llm_decision import LLMDecision
...
class FuturesTrader:
    ...
    def setup_leverage(self, symbol: str, leverage: int) -> None:
        if self._cfg.dry_run:
            ...
            return
        result = self._client.change_leverage(symbol, leverage)
    ...
    def execute(
        ...
        if decision.action is Action.HOLD:
            if position.is_open and (decision.sl or decision.tp):
                ...
                sl, tp = self._validate_sl_tp(
                    is_long, current_price, decision.sl, decision.tp
                )
                logger.info(
                    "[%s] HOLD — updating SL/TP on existing %s position",
                    symbol,
                    position.side,
                )
        ...
        if decision.action is Action.BUY:
            if position.side == "LONG":
                logger.info(
                    "[%s] Already LONG — skipping entry, updating SL/TP only.", symbol
                )
            ...
            if position.side == "SHORT":
                logger.info(
                    "[%s] Already SHORT — skipping entry, updating SL/TP only.", symbol
                )
    ...
    def _stop_order(
        ...
        try:
            ...
            logger.error(
                "Failed to place %s order at %s: %s", label, rounded_price, exc
            )
    ...
    def _market_order(self, symbol: str, side: str, quantity: float) -> str | None:
        ...
        if float(rounded_qty) <= 0:
            logger.error(
                "Computed quantity rounds to zero for %s — skipping order.", symbol
            )
            return None
    ...
    def _ensure_precision(self, symbol: str) -> None:
        ...
        if symbol in self._precision:
            return
        try:
            # UM futures exchangeInfo has no symbol filter — filter locally.
            ...
            for sym_info in info.get("symbols", []):
                if sym_info["symbol"] == symbol:
                    ...
                    logger.debug(
                        "Precision for %s: price=%d qty=%d", symbol, price_dec, qty_dec
                    )
                    return
        except Exception as exc:
            logger.warning(
                "Could not fetch exchange info for %s: %s — using defaults", symbol, exc
            )
    ...
    def _calc_quantity(
        ...
        if price is None or price <= 0:
            try:
                ticker = self._client.ticker_24hr_price_change(symbol=symbol)
