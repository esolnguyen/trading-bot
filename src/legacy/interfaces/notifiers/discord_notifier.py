...
from src.mcp_servers.shared.utils.decorators import retry_async
...
if TYPE_CHECKING:
    ...
    from src.mcp_servers.shared.utils.format_utils import FormatUtils
...
class DiscordNotifier(BaseNotifier):
    ...
    def __init__(
        ...
        Args:
            ...
            config: Settings instance for Discord settings
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                "Discord Login Failure: %s. Check your BOT_TOKEN_DISCORD.",
                e,
                exc_info=True,
            )
        except Exception as e:
    ...
    async def send_message(
        self, message: str, channel_id: int, expire_after: Optional[int] = None
        ...
        try:
            ...
            if len(message) > MAX_TOTAL_LENGTH:
                self.logger.warning(
                    "Message length (%s) exceeds maximum (%s). Truncating.",
                    len(message),
                    MAX_TOTAL_LENGTH,
                )
            ...
            for i, chunk in enumerate(chunks):
                ...
                sent_message = await channel.send(
                    content=chunk, delete_after=delete_after
                )
            ...
            self.logger.debug(
                "Sent %s message chunk(s) (Last ID: %s)",
                len(chunks),
            ...
            self.logger.error(
                "Discord HTTPException when sending message: %s", e, exc_info=True
            )
        except Exception as e:
            self.logger.error(
                "Unexpected error when sending message: %s", e, exc_info=True
            )
        return None
    ...
    async def send_trading_decision(self, decision: Any, channel_id: int) -> None:
        ...
        try:
            ...
            if decision.stop_loss:
                embed.add_field(
                    name="Stop Loss", value=f"${decision.stop_loss:,.2f}", inline=True
                )
            if decision.take_profit:
                embed.add_field(
                    name="Take Profit",
                    value=f"${decision.take_profit:,.2f}",
                    inline=True,
                )
            if decision.position_size:
                embed.add_field(
                    name="Position Size",
                    value=f"{decision.position_size * 100:.2f}%",
                    inline=True,
                )
            if decision.quote_amount:
                embed.add_field(
                    name="Invested", value=f"${decision.quote_amount:,.2f}", inline=True
                )
            if decision.quantity:
                embed.add_field(
                    name="Quantity",
                    value=self.formatter.fmt(decision.quantity),
                    inline=True,
                )
            if decision.action in ENTRY_ACTIONS and decision.quantity:
                ...
                embed.add_field(
                    name="Entry Fee", value=f"${entry_fee:.4f}", inline=True
                )

            embed.set_footer(
                text=f"Time: {decision.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    async def send_analysis_notification(
        self, result: dict, symbol: str, timeframe: str, channel_id: int
    ) -> None:
        ...
        try:
            ...
            reasoning = (
                self.unified_parser.extract_text_before_json(raw_response)
                if raw_response
                else ""
            )
    ...
    async def send_position_status(
        self, position: Any, current_price: float, channel_id: int
    ) -> None:
        ...
        try:
            ...
            stop_distance_pct, target_distance_pct = (
                self.calculate_stop_target_distances(position, current_price)
            )
            ...
            embed.add_field(
                name="Quantity", value=self.formatter.fmt(position.size), inline=True
            )
            try:
                if position.quote_amount > 0:
                    embed.add_field(
                        name="Invested",
                        value=f"${position.quote_amount:,.2f}",
                        inline=True,
                    )
            ...
            embed.add_field(
                name="Unrealized P&L", value=f"{pnl_pct:+.2f}%", inline=True
            )
            embed.add_field(
                name=f"P&L ({self.config.quote_currency})",
                value=f"${pnl_quote:+,.2f}",
                inline=True,
            )
            embed.add_field(name="Confidence", value=position.confidence, inline=True)
            embed.add_field(
                name="Position Size %",
                value=f"{position.size_pct * 100:.2f}%",
                inline=True,
            )
            embed.add_field(
                name="Stop Loss",
                value=f"${position.stop_loss:,.2f} ({stop_distance_pct:+.2f}%)",
                inline=True,
            )
            embed.add_field(
                name="Take Profit",
                value=f"${position.take_profit:,.2f} ({target_distance_pct:+.2f}%)",
                inline=True,
            )
            embed.add_field(
                name="Entry Fee", value=f"${position.entry_fee:.4f}", inline=True
            )
            embed.add_field(name="Time Held", value=f"{hours_held:.1f}h", inline=True)
            embed.set_footer(
                text=f"Entry Time: {position.entry_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            await self._send_embed(embed, channel_id)
        except Exception as e:
    ...
    async def send_performance_stats(
        self, trade_history: List[Dict[str, Any]], symbol: str, channel_id: int
    ) -> None:
        ...
        try:
            ...
            embed.add_field(
                name=f"Total P&L ({self.config.quote_currency})",
                value=f"${stats['total_pnl_quote']:+,.2f}",
                inline=True,
            )
            embed.add_field(
                name="Total P&L (%)",
                value=f"{stats['total_pnl_pct']:+.2f}%",
                inline=True,
            )
            embed.add_field(
                name="Avg P&L/Trade", value=f"{stats['avg_pnl_pct']:+.2f}%", inline=True
            )
            embed.add_field(
                name="Win Rate",
                value=f"{stats['win_rate']:.1f}% ({stats['winning_trades']}/{stats['closed_trades']})",
                inline=True,
            )
            embed.add_field(
                name="Total Trades", value=str(stats["closed_trades"]), inline=True
            )
            embed.add_field(
                name="Total Fees", value=f"${stats['total_fees']:.4f}", inline=True
            )
            embed.add_field(
                name=f"Net P&L ({self.config.quote_currency})",
                value=f"${stats['net_pnl']:+,.2f}",
                inline=True,
            )
    ...
    def _create_analysis_embed(
        self, analysis: dict, symbol: str, timeframe: str
        ...
        try:
            ...
            if fields["entry_price"]:
                embed.add_field(
                    name="Entry", value=f"${fields['entry_price']:,.2f}", inline=True
                )
            if fields["stop_loss"]:
                embed.add_field(
                    name="Stop Loss", value=f"${fields['stop_loss']:,.2f}", inline=True
                )
            if fields["take_profit"]:
                embed.add_field(
                    name="Take Profit",
                    value=f"${fields['take_profit']:,.2f}",
                    inline=True,
                )

            embed.add_field(
                name="Confidence", value=f"{fields['confidence']}%", inline=True
            )

            if fields["risk_reward_ratio"]:
                embed.add_field(
                    name="R:R", value=f"{fields['risk_reward_ratio']:.2f}", inline=True
                )
            ...
            if trend:
                ...
                embed.add_field(
                    name="Trend", value=f"{direction} ({strength}%)", inline=True
                )
            ...
            if key_levels:
                ...
                if resistances:
                    ...
                    embed.add_field(
                        name="Resistance", value=resistance_str, inline=False
                    )
