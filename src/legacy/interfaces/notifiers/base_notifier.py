...
class BaseNotifier(ABC):
    ...
    def calculate_stop_target_distances(
        ...
        if position.direction == "LONG":
            stop_distance_pct = (
                (position.stop_loss - current_price) / current_price
            ) * 100
            target_distance_pct = (
                (position.take_profit - current_price) / current_price
            ) * 100
        else:
            stop_distance_pct = (
            ...
            target_distance_pct = (
                (current_price - position.take_profit) / current_price
            ) * 100
        ...
        for decision_dict in trade_history:
            ...
            if action in ENTRY_ACTIONS:
                ...
                if entry_fee == 0.0 and open_quantity > 0:
                    entry_fee = (
                        open_price * open_quantity * self.config.transaction_fee_percent
                    )
    ...
    def parse_analysis_response(self, raw_response: str) -> Tuple[str, Optional[dict]]:
        ...
        analysis_json = self.unified_parser.extract_json_block(
            raw_response, unwrap_key="analysis"
        )
