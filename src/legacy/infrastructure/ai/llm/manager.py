...
from src.legacy.domain.trading import TradeDecision
from .azure import (
...
class LLMManager:
    ...
    async def decide(
        ...
        self.last_prompt = self._build_log_prompt(
            system_prompt, user_message, chart_b64
        )
    ...
    async def send_prompt(
        ...
        if self.settings.provider == "azure":
            client = self._get_azure_client()
            response = await complete_azure(
                client, self.settings.azure_deployment, messages
            )
    ...
    async def send_prompt_with_chart_analysis(
        ...
        result = await orch.get_chart_response(
            effective_provider, msgs, chart_image, model
        )
