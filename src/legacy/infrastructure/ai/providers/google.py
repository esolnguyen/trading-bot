...
class GoogleAIClient(BaseAIClient):
    ...
    def _extract_text_from_response(self, response) -> str:
        ...
        try:
            ...
            if non_text_parts:
                self.logger.debug(
                    "Google AI response contains non-text parts: %s. Extracting text only.",
                    non_text_parts,
                )
            return "\n".join(text_parts)
    ...
    def _create_generation_config(
        ...
        if include_thinking:
            ...
            if thinking_level and thinking_level in (
                "minimal",
                "low",
                "medium",
                "high",
    ...
    async def chat_completion(
        ...
        for include_thinking in (True, False):
            try:
                ...
                self.logger.debug(
                    "Sending request to Google AI with model: %s (thinking=%s)",
                    effective_model,
                    include_thinking,
                )
                response = await client.aio.models.generate_content(
                    model=effective_model, contents=prompt, config=generation_config
                )
                ...
                if include_thinking and (
                    ...
                    self.logger.warning(
                        "Model may not support thinking_config, retrying without it: %s",
                        e,
                    )
                    continue
    ...
    async def chat_completion_with_chart_analysis(
        ...
        for include_thinking in (True, False):
            try:
                ...
                include_code_execution = model_config.get(
                ...
                response = await client.aio.models.generate_content(
                    model=effective_model, contents=contents, config=generation_config
                )
                content_text = self._extract_text_from_response(response)
                usage = self._extract_usage_metadata(response)
                self.logger.debug(
                    "Received successful chart analysis response from Google AI"
                )
                return self.create_response(content_text, usage=usage)
            except Exception as e:  # pylint: disable=broad-exception-caught
                error_str = str(e).lower()
                if include_thinking and (
                    ...
                    self.logger.warning(
                        "Model may not support thinking_config for chart analysis, retrying without it: %s",
                        e,
                    )
                    continue
                self.logger.error(
                    "Error during Google AI chart analysis request: %s", e
                )
                return self._handle_exception(e)
        return None
