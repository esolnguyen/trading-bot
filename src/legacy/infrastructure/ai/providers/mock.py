...
class MockClient(BaseAIClient):
    ...
    def __init__(
        ...
        super().__init__(logger or logging.getLogger("mock_client"))
    ...
    def _synthesize_response(
        self, last_close: Optional[float] = None, has_chart: bool = False
    ) -> str:
        ...
        payload = {
            "analysis": {
                ...
                "key_levels": {
                    "support": [round(entry * 0.98, 2)],
                    "resistance": [round(entry * 1.02, 2)],
                },
                "trend": {
                    "direction": (
                        "BULLISH"
                        if signal == "BUY"
                        else ("BEARISH" if signal == "SELL" else "NEUTRAL")
                    ),
                    "strength": confidence,
