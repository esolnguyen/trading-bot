...
class BlockRunClient(BaseAIClient):
    ...
    def __init__(self, wallet_key: str, base_url: str, logger: logging.Logger) -> None:
    ...
    async def _initialize_client(self) -> None:
        ...
        try:
            ...
            self._client = AsyncLLMClient(
                private_key=self._wallet_key, api_url=self.base_url
            )
            self.logger.debug("BlockRun SDK client initialized successfully")
        except ImportError:
            self.logger.error(
                "BlockRun SDK not installed. Run: pip install blockrun-llm"
            )
            raise
    ...
    def _redact_private_key(self, message: str) -> str:
        ...
        if self._wallet_key and len(self._wallet_key) > 10:
            return message.replace(
                self._wallet_key, f"{self._wallet_key[:6]}...{self._wallet_key[-4:]}"
            )
        return message
    ...
    async def chat_completion_with_chart_analysis(
        ...
        try:
            ...
            self.logger.debug(
                "Sending chart analysis request to BlockRun SDK (%s bytes)",
                len(img_data),
            )
            effective_model = self._ensure_provider_prefix(model)
            ...
            if response:
                self.logger.debug(
                    "Received successful chart analysis response from BlockRun SDK"
                )
            return self.convert_pydantic_response(response, wrapper_attr="response")
        except Exception as e:
            self.logger.error(
                "Error during BlockRun chart analysis request: %s",
                self._redact_private_key(str(e)),
            )
            return self._handle_exception(e)
    ...
    def _extract_all_user_text_from_messages(
