...
import src.mcp_servers.shared.infrastructure.ai.providers.lmstudio as lms
...
from src.mcp_servers.shared.utils.decorators import retry_api_call
...
class LMStudioClient(BaseAIClient):
    ...
    async def close(self) -> None:
        ...
        if self._client:
            self.logger.debug(
                "LMStudioClient SDK session does not require explicit closing."
            )
    ...
    async def _get_model_or_auto_select(
        self, client: lms.AsyncClient, model: str
    ) -> str:
    ...
    async def chat_completion(
        ...
        try:
            async with lms.AsyncClient(api_host=api_host) as client:
                ...
                self.logger.debug(
                    "Sending request to LM Studio SDK with model: %s (host=%s)",
                    model,
                    api_host,
                )
    ...
    async def chat_completion_with_chart_analysis(
        ...
        try:
            ...
            async with lms.AsyncClient(api_host=api_host) as client:
                model = await self._get_model_or_auto_select(client, model)
                self.logger.debug(
                    "Sending chart analysis request to LM Studio SDK with model: %s (host=%s)",
                    model,
                    api_host,
                )
                ...
                response = await llm.respond(chat, config=config)
                self.logger.debug(
                    "Received successful chart analysis response from LM Studio SDK"
                )
                content = str(response) if response else ""
                return self.create_response(
                    content=content,
            ...
            self.logger.error(
                "Error during LM Studio chart analysis request: %s", str(e)
            )
            return self._handle_exception(e)

    @retry_api_call(max_retries=3, initial_delay=1, backoff_factor=2, max_delay=30)
    async def stream_chat_completion(
        self, model: str, messages: list, model_config: Dict[str, Any], callback=None
    ) -> Optional[ChatResponseModel]:
        ...
        try:
            async with lms.AsyncClient(api_host=api_host) as client:
                model = await self._get_model_or_auto_select(client, model)
                self.logger.debug(
                    "Sending streaming request to LM Studio SDK with model: %s (host=%s)",
                    model,
                    api_host,
                )
                llm = await client.llm.model(model)
    ...
    ) -> Optional[lms.LlmPredictionConfig]:
                ...
                try:
                    ...
                    if "unexpected keyword argument" in error_str.lower():
                        ...
                        if match:
                            ...
                            self.logger.warning(
                                "LM Studio SDK rejected parameter '%s', retrying without it.",
                                bad_arg,
                            )
                            del config_dict[bad_arg]
                            continue
                    self.logger.warning(
                        "Failed to build LlmPredictionConfig: %s. Falling back to default config.",
                        e,
                    )
                    break
        return None
