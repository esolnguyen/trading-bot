...
from src.mcp_servers.shared.infrastructure.ai.provider_types import (
    ProviderMetadata,
    InvocationResult,
    ProviderClients,
)
...
class ProviderOrchestrator:
    ...
    def __init__(
        self, logger: logging.Logger, settings: Settings, clients: ProviderClients
    ) -> None:
        """
    ...
    def _build_provider_metadata(self) -> Dict[str, ProviderMetadata]:
        """Build provider metadata registry from clients and settings."""
        return {
            "googleai": ProviderMetadata(
                name="Google AI Studio",
                client=self.clients.google,
                paid_client=self.clients.google_paid,
                default_model=self.settings.google_studio_model,
                config=self.settings.get_model_config(
                    self.settings.google_studio_model
                ),
                supports_chart=True,
            ...
            "openrouter": ProviderMetadata(
                ...
                default_model=self.settings.openrouter_base_model,
                config=self.settings.get_model_config(
                    self.settings.openrouter_base_model
                ),
                supports_chart=True,
                has_rate_limits=True,
            ),
            "local": ProviderMetadata(
                ...
                config=self.settings.get_model_config(self.settings.lm_studio_model),
            ...
            "blockrun": ProviderMetadata(
                ...
                default_model=self.settings.blockrun_model,
                config=self.settings.get_model_config(self.settings.blockrun_model),
                supports_chart=True,
                has_rate_limits=False,
            ),
        }
    ...
    def supports_chart(self, provider: str) -> bool:
        ...
        metadata = self._providers.get(provider)
        return (
            metadata.supports_chart if metadata and metadata.is_available() else False
        )
        ...
        *,
        ...
        if not metadata or not metadata.is_available():
            return InvocationResult(
                success=False,
                response=ChatResponseModel.from_error(
        ...
        if provider == "googleai":
            return await self._invoke_google(
                metadata, messages, effective_model, chart, chart_image
            )
        if provider == "local":
        ...
        if provider == "blockrun":
            return await self._invoke_blockrun(
                metadata, messages, effective_model, chart, chart_image
            )
        return InvocationResult(
            success=False,
        ...
        *,
        ...
        for provider in providers:
            ...
            result = await self.invoke(
                provider, messages, chart=chart, chart_image=chart_image, model=model
            )
    ...
    async def get_text_response(
        ...
        if effective_provider == "all":
            ...
            if not result.success and self.is_available("openrouter"):
                self.logger.warning(
                    "Google AI Studio and LM Studio failed. Final fallback to OpenRouter..."
                )
        ...
        if self.is_available(effective_provider):
            ...
            self.logger.info(
                "Using %s model: %s",
                self._providers[effective_provider].name,
                effective_model,
            )
        ...
        return InvocationResult(
            success=False,
            response=ChatResponseModel.from_error(
                f"Provider '{effective_provider}' is not available"
            ),
    ...
    async def get_chart_response(
        ...
        if effective_provider == "local":
            return InvocationResult(
                success=False,
                response=ChatResponseModel.from_error(
                    "Chart analysis unavailable - local models don't support images"
                ),
                provider="local",
        ...
        return InvocationResult(
            success=False,
            response=ChatResponseModel.from_error(
                f"Provider '{effective_provider}' is not available for chart analysis"
            ),
            provider=effective_provider,
            model=self.resolve_model(effective_provider, model),
        )
    ...
    async def _invoke_google(
        ...
        self.logger.info(
            "Attempting with Google AI %s API (model: %s)", tier_info, effective_model
        )
        if chart and chart_image:
            ...
            response = await metadata.client.chat_completion(
                effective_model, messages, metadata.config
            )
        error_type = response.error if response else None
        if (
            error_type
            and ("overloaded" in error_type or "rate_limit" in error_type)
            and metadata.paid_client
        ):
            error_reason = (
                "rate limited" if error_type == "rate_limit" else "overloaded"
            )
            self.logger.warning(
                "Google AI free tier %s, retrying with paid API key", error_reason
            )
            if chart and chart_image:
                response = (
                    await metadata.paid_client.chat_completion_with_chart_analysis(
                        effective_model,
                        messages,
                        cast(Any, chart_image),
                        metadata.config,
                    )
                )
            else:
                response = await metadata.paid_client.chat_completion(
                    effective_model, messages, metadata.config
                )
            if self._is_valid_response(response):
                self.logger.info(
                    "Successfully used paid Google AI API after free tier %s",
                    error_reason,
                )
                return InvocationResult(
                    success=True,
        ...
        return InvocationResult(
            success=False, response=response, provider="google", model=effective_model
        )
    ...
    async def _invoke_openrouter(
        ...
        success = self._is_valid_response(response) and not self._is_rate_limited(
            response
        )
        return InvocationResult(
            success=success,
    ...
    def _is_valid_response(self, response: Optional[ChatResponseModel]) -> bool:
        ...
        if first_choice.error:
            ...
            provider = (
                error_detail.get("metadata", {}).get("provider_name", "unknown")
                if isinstance(error_detail, dict)
                else "unknown"
            )
            self.logger.error(
                "Error in API response choice from %s: [%s] %s",
                provider,
                error_code,
                error_msg,
            )
        ...
        if not content:
            self.logger.debug(
                "Empty content in API response choice. Message: %s",
    ...
    def _log_unavailable_guidance(self, provider: str) -> None:
        ...
        if not metadata or not metadata.client:
            if provider == "openrouter":
                self.logger.error(
                    "OpenRouter client not initialized. Check OPENROUTER_API_KEY in keys.env"
                )
            elif provider == "googleai":
                self.logger.error(
                    "Google AI client not initialized. Check GOOGLE_STUDIO_API_KEY in keys.env"
                )
            elif provider == "local":
                self.logger.error(
                    "LM Studio client not initialized. Check LM_STUDIO_BASE_URL in config.ini"
                )
