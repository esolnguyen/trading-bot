...
from src.mcp_servers.shared.utils.json_io import (
...
class Persistence:
    ...
    def append_trade(self, outcome: TradeOutcome, timestamp_iso: str) -> None:
        write_header = (
            not self.trades_csv_path.exists()
        ...
        with self.trades_csv_path.open("a", encoding="utf-8", newline="") as handle:
            ...
            writer.writerow(
                [
                    ...
                    (
                        outcome.decision.price
                        if outcome.decision.price is not None
                        else ""
                    ),
    ...
    def append_trade_event(
        ...
        *,
        ...
        line = self._format_trade_event(
            outcome, timestamp_iso, position_before, position_after
        )
        if line is None:
            return
    ...
    def save_position(
        self, symbol: str, position_data: Optional[dict[str, Any]]
    ) -> None:
    ...
    async def async_save_trade_record(
        self, symbol: str, record: dict[str, Any]
    ) -> None:
    ...
    def save_last_analysis_time(
        ...
        self._write_json(
            self._analysis_time_path(symbol), {"timestamp": ts.isoformat()}
        )
    ...
    async def async_save_last_analysis_time(
        self, symbol: str, ts: Optional[datetime] = None
    ) -> None:
