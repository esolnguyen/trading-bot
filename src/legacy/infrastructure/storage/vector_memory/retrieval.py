...
from src.mcp_servers.shared.domain.analysis.stats import VectorSearchResult
                ...
                rules.append(
                    {
                        "rule_id": rule_id,
                        "text": (
                            all_rules["documents"][i] if all_rules["documents"] else ""
                        ),
                        "metadata": (
                            all_rules["metadatas"][i] if all_rules["metadatas"] else {}
                        ),
                    }
                )
        return rules
...
def get_trade_metadatas(
    service: Any, exclude_updates: bool = True
) -> List[Dict[str, Any]]:
