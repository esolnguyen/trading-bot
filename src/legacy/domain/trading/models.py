...
class OpenPosition:
    """Snapshot of the current futures position for a single symbol (llm_trader path)."""
...
class TradingMemory(SerializableMixin):
    ...
    def get_context_summary(
        self, full_history: Optional[List[TradeRecord]] = None
    ) -> str:
        ...
        history_to_analyze = sorted(
            history_to_analyze, key=lambda x: _ensure_utc(x.timestamp)
        )
        ...
        for decision in history_to_analyze:
            if decision.action in ("BUY", "SELL"):
                ...
                decision.action in ("CLOSE", "CLOSE_LONG", "CLOSE_SHORT")
                and open_position
            ):
                if open_position.action == "BUY":
                    pnl_pct = (
                        (decision.price - open_position.price) / open_position.price
                    ) * 100
                    pnl_quote = (
                        decision.price - open_position.price
                    ) * open_position.quantity
                else:
                    pnl_pct = (
                        (open_position.price - decision.price) / open_position.price
                    ) * 100
                    pnl_quote = (
                        open_position.price - decision.price
                    ) * open_position.quantity
        ...
        if closed_trades > 0:
            ...
            lines.append("")
            lines.append(
                f"## Overall Performance ({closed_trades} Total Closed Trades):"
            )
            lines.append(
                f"- Total P&L: ${total_pnl_quote:+,.2f} ({total_pnl_pct:+.2f}%)"
            )
            lines.append(f"- Average P&L per Trade: {avg_pnl_pct:+.2f}%")
            lines.append(
                f"- Win Rate: {win_rate:.1f}% ({winning_trades}/{closed_trades} trades)"
            )
    ...
    @classmethod
    def from_list(
        cls, data: List[Dict[str, Any]], max_decisions: int = 10
    ) -> "TradingMemory":
