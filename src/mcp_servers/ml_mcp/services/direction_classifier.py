...
class DirectionClassifier:
    ...
    def predict_proba(
        self, indicators: Any, price: float | None = None, symbol: str | None = None
    ) -> float | None:
    ...
    def format_context(
        ...
        conviction = (
            "high"
            if abs(prob - 0.5) > 0.2
            else "moderate" if abs(prob - 0.5) > 0.1 else "weak"
        )
        return (
    ...
    def _indicators_to_row(
        ...
        mapping["ema_20_dist"] = (
            (ref_price - mapping["ema_20"]) / mapping["ema_20"] * 100
            if mapping["ema_20"] > 0
            else 0.0
        )
        mapping["ema_50_dist"] = (
            (ref_price - mapping["ema_50"]) / mapping["ema_50"] * 100
            if mapping["ema_50"] > 0
            else 0.0
        )
        mapping["atr_pct"] = (
            mapping["atr"] / price * 100
            if price and price > 0
            else (
                mapping["atr"] / mapping["bb_mid"] * 100
                if mapping["bb_mid"] > 0
                else 0.0
            )
        )
