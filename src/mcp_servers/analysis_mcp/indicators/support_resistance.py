...
from src.mcp_servers.analysis_mcp.indicators.trend import supertrend_numba
...
def support_resistance_numba_advanced(high, low, close, volume, length):
    ...
    for i in range(length, n):
        rolling_avg_volume[i], volume_filter[i] = _calculate_volume_filter_numba(
            volume, length, i
        )
...
@njit(cache=True)
def advanced_support_resistance_numba(
    high,
    low,
    close,
    volume,
    ...
    for i in range(length, n):
        ...
        if volume_filter[i] and volume[i] > volume_factor * rolling_avg_volume[i]:
            if (
                support_strength[i] >= strength_threshold
                and close[i] < (1 - price_factor) * s1[i]
            ):
                for j in range(max(0, i - persistence + 1), i + 1):
                    strong_support[j] = min(s1[j], s2[j])
            if (
                resistance_strength[i] >= strength_threshold
                and close[i] > (1 + price_factor) * r1[i]
            ):
                for j in range(max(0, i - persistence + 1), i + 1):
...
def find_support_resistance_numba(close, support, resistance, window):
    ...
    if len(valid_support) > 0:
        nearest_support = (
            np.max(valid_support[valid_support < current_price])
            if np.any(valid_support < current_price)
            else np.min(valid_support)
        )
    ...
    if len(valid_resistance) > 0:
        nearest_resistance = (
            np.min(valid_resistance[valid_resistance > current_price])
            if np.any(valid_resistance > current_price)
            else np.max(valid_resistance)
        )
...
def fibonacci_bollinger_bands_numba(src, volume, length, mult):
    ...
    for i in range(len(src)):
        if i < length:
            ...
            sum_pv = np.sum(
                src[i - length + 1 : i + 1] * volume[i - length + 1 : i + 1]
            )
