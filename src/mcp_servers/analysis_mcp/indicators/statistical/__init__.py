...
@njit(cache=True)
def apa_adaptive_eot_numba(
    closeprices, q1_=0.8, q2_=0.4, minlen=10, maxlen=48, avelen=3
):
...
def skew_numba(close, length=30):
    ...
    if std_dev > 1e-10:
        skew_values[length - 1] = bias_factor * (
            sum_cubed_diff / (std_dev * std_dev * std_dev)
        )
    else:
...
def calculate_eot_numba(close_prices, period=21, q1=0.8, q2=0.4):
    ...
    for i in range(period, n):
        hp[i] = (
            (1 - alpha1 / 2) ** 2
            * (close_prices[i] - 2 * close_prices[i - 1] + close_prices[i - 2])
            + 2 * (1 - alpha1) * hp[i - 1]
            - (1 - alpha1) ** 2 * hp[i - 2]
        )
