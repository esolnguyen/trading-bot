...
@njit(cache=True)
def _calculate_macd_window(
    close, macd_fast_length, macd_slow_length, macd_signal_length
):
...
def _calculate_mfi_window(high, low, close, volume, mfi_length):
    ...
    for i in range(mfi_length, window_size):
        pmf = np.sum(
            rmf[i - mfi_length + 1 : i + 1][
                tp[i - mfi_length + 1 : i + 1] > tp[i - mfi_length : i]
            ]
        )
        nmf = np.sum(
            rmf[i - mfi_length + 1 : i + 1][
...
@njit(cache=True)
def _calculate_fear_greed_for_window(
    rsi_list,
    histogram_list,
    mfi_list,
    window_size,
    ...
    for i in range(start_idx, window_size):
        ...
        normalized_macd_histogram = _normalize_value(
            histogram_list[i], min_histogram, max_histogram
        )
...
@njit(cache=True)
def _fear_and_greed_index_numba(
    close,
    high,
    low,
    volume,
    ...
    for start in range(n - window_size + 1):
        ...
        mfi_list = _calculate_mfi_window(
            window_high, window_low, window_close, window_volume, mfi_length
        )
