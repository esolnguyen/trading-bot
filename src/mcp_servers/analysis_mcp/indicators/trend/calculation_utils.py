...
def calculate_directional_indicators(
    dm_pos14: np.ndarray, dm_neg14: np.ndarray, tr14: np.ndarray
...
@njit(cache=True)
def calculate_ichimoku_lines(
    high: np.ndarray, low: np.ndarray, conversion_length: int, base_length: int
) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(conversion_length - 1, n):
        conversion_line[i] = (
            np.max(high[i - conversion_length + 1 : i + 1])
            + np.min(low[i - conversion_length + 1 : i + 1])
        ) / 2

    # Calculate Base Line (Kijun-sen)
    for i in range(base_length - 1, n):
        base_line[i] = (
            np.max(high[i - base_length + 1 : i + 1])
            + np.min(low[i - base_length + 1 : i + 1])
        ) / 2
...
def calculate_ichimoku_spans(
    high: np.ndarray,
    low: np.ndarray,
    conversion_line: np.ndarray,
    base_line: np.ndarray,
    lagging_span2_length: int,
    displacement: int,
) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(lagging_span2_length - 1, n - displacement):
        leading_span_b[i + displacement] = (
            np.max(high[i - lagging_span2_length + 1 : i + 1])
            + np.min(low[i - lagging_span2_length + 1 : i + 1])
        ) / 2
...
@njit(cache=True)
def calculate_band_adjustments(
    close: np.ndarray, upperband: np.ndarray, lowerband: np.ndarray, i: int
) -> Tuple[float, float]:
...
@njit(cache=True)
def calculate_vortex_components(
