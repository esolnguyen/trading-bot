...
def mfi_numba(high, low, close, volume, length=14, drift=1):
    ...
    for i in range(length, n):
        ...
        for j in range(i - length + 1, i + 1):
            tp_diff = tp[j] - (
                (high[j - drift] + low[j - drift] + close[j - drift]) / 3
            )
...
def chaikin_money_flow_numba(high, low, close, volume, length):
    ...
    for i in range(length - 1, n):
        ...
        for j in range(i - length + 1, i + 1):
            if high[j] != low[j]:
                money_flow_multiplier = ((close[j] - low[j]) - (high[j] - close[j])) / (
                    high[j] - low[j]
                )
...
def ad_line_numba(high, low, close, volume):
    ...
    for i in range(1, n):
        if high[i] != low[i]:
            money_flow_multiplier = ((close[i] - low[i]) - (high[i] - close[i])) / (
                high[i] - low[i]
            )
...
def eom_numba(high, low, volume, length=14, divisor=10000.0, drift=1):
    ...
    for i in range(drift, n):
        ...
        if hl_range == 0:
            ...
            distance = ((high[i] + low[i]) / 2) - (
                (high[i - drift] + low[i - drift]) / 2
            )
...
def volume_profile_numba(close, volume, length, num_bins):
    ...
    for i in range(length, n):
        ...
        price_range = np.linspace(
            np.min(window_close), np.max(window_close), num_bins + 1
        )
        volume_profile = np.zeros(num_bins)

        for j in range(num_bins):
            mask = (window_close >= price_range[j]) & (
                window_close < price_range[j + 1]
            )
