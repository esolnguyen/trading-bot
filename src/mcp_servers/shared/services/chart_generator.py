...
class ChartGenerator:
    ...
    def _render_with_mplfinance(
        ...
        add_plots = [
            ...
            mpf.make_addplot(
                macd_hist_series, type="bar", panel=3, color="lightgreen", ylabel="MACD"
            ),
        ]
    ...
    def _render_with_matplotlib(
        ...
        colors = [
            "#49d17d" if candle.close >= candle.open else "#ff5c7a"
        ...
        for index, candle in enumerate(candles):
            ...
            main_axis.bar(
                index, body_height, bottom=body_low, color=colors[index], width=0.6
            )
        ...
        rsi_axis.plot(
            indices, [indicators.rsi_14] * len(indices), color="#5ad1ff", linewidth=1.5
        )
        ...
        macd_axis.bar(
            indices, [indicators.macd_hist] * len(indices), color="#8bd450", width=0.8
        )
    ...
    def _figure_to_base64_png(figure) -> str:
        ...
        buffer = BytesIO()
        figure.savefig(
            buffer, format="png", facecolor=figure.get_facecolor(), bbox_inches="tight"
        )
        plt.close(figure)
        buffer.seek(0)
