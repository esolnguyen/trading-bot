...
from src.mcp_servers.shared.domain.analysis import PatternResult
...
class PatternAnalyzer:
    ...
    def _has_double_bottom(
        self, candles: list[OHLCVCandle], leg_sep: int, recency: int
    ) -> bool:
    ...
    def _local_extrema(
        self, candles: list[OHLCVCandle], kind: str
        ...
        for index in range(1, len(candles) - 1):
            ...
            if kind == "max":
                if (
                    ...
                    and (
                        candle.high > prev_candle.high or candle.high > next_candle.high
                    )
                ):
                    extrema.append((index, candle.high))
        return extrema

    def _find_repeated_level(
        self, candles: list[OHLCVCandle], *, kind: str
    ) -> float | None:
        if len(candles) < 3:
            return None

        extrema = self._local_extrema(
            candles, kind="min" if kind == "support" else "max"
        )
        if len(extrema) < 3:
            return None
        ...
        for _index, level in extrema:
            if not any(
                abs(level - existing) <= existing * 0.01 for existing in touches
            ):
