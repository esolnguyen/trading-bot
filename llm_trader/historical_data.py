...
Reuses the existing Binance Futures REST endpoint that ``MarketDataClient``
uses live, but walks ``startTime``/``endTime`` to pull arbitrarily long
ranges. Logic adapted from Vibe-Trading's ccxt loader (paginated cursor
walk), but kept on the same Binance transport llm_trader already uses
...
from .market_data import Candle, MarketDataClient, _ms_to_iso

logger = logging.getLogger(__name__)

# Binance interval -> milliseconds per bar (covers what llm_trader uses)
_INTERVAL_MS = {
    "1m": 60_000,
    "3m": 180_000,
    "5m": 300_000,
