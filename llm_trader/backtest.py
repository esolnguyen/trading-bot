"""LLM-in-the-loop backtest harness for llm_trader.
...
The fee/slippage/funding/liquidation formulas are lifted verbatim from
HKUDS Vibe-Trading's ``backtest/engines/crypto.py`` (tiered maintenance
margin table, 8h funding schedule, side-aware slippage).

Vibe-Trading's ``BaseEngine`` is signal-driven (vectorised target
weights), which is the wrong shape for an LLM that decides one bar at a
time, so this harness is a small custom driver around the same physics.

Usage:

    python -m llm_trader.backtest --symbol BTCUSDT --days 30
    python -m llm_trader.backtest --symbol ETHUSDT --start 2025-01-01 --end 2025-02-01 --mock
"""
...
from .llm_decision import Action, LLMDecision, LLMDecisionEngine
from .market_data import Candle, MarketDataClient
from .metrics import Trade, calc_metrics
...
# ── Tiered maintenance margin (lifted from Vibe-Trading crypto.py) ───────────
...
# ── Internal mutable position state (separate from frozen OpenPosition) ──────
...
class Backtester:
    ...
    def __init__(
        ...
        if len(candles) <= warmup:
            raise ValueError(
                f"Need more than {warmup} candles, got {len(candles)}"
            )
        self.cfg = cfg
        self.symbol = symbol
    ...
    def _snapshot_position(self, mark: float) -> OpenPosition:
        if self.pos is None:
            return OpenPosition(
                side="NONE", entry_price=0.0, quantity=0.0,
                unrealized_pnl=0.0, liquidation_price=0.0,
                current_sl=None, current_tp=None,
            )
        return OpenPosition(
    ...
    def _open(
        self, direction: int, bar: Candle, ts: str, bar_idx: int,
    ...
    def _close(
        self, exit_price: float, exit_ts: str, reason: str, bar_idx: int,
    ) -> None:
        if self.pos is None:
            return
        slipped = _slip(exit_price, -self.pos.direction, self.cfg.backtest_slippage)
        pnl_gross = self.pos.direction * self.pos.size * (slipped - self.pos.entry_price)
    ...
    def _check_sl_tp(self, bar: Candle, ts: str, bar_idx: int) -> bool:
        ...
        if sl_hit and tp_hit:
            # Conservative: assume SL hit first (worst case for the strategy)
    ...
    def _check_liquidation(self, bar: Candle, ts: str, bar_idx: int) -> None:
        ...
        if self.pos.margin + unrealized <= maint:
            liq_price = _slip(bar.close, -self.pos.direction, self.cfg.backtest_slippage)
            self._close(liq_price, ts, "liquidation", bar_idx)

    # ── Decision handling ──

    def _apply_decision(
        self, decision: LLMDecision, bar: Candle, ts: str, bar_idx: int,
    ) -> None:
        target_dir = (
            1 if decision.action is Action.BUY
            else -1 if decision.action is Action.SELL
            else 0
        )
        ...
        if self.pos is not None:
            if target_dir == 0:
                # HOLD — refresh SL/TP if model gave new ones
            ...
            if target_dir != self.pos.direction:
                # Flip — close at this bar's open, then open opposite
                self._close(bar.open, ts, "signal", bar_idx)
            else:
                # Same side — refresh SL/TP only
                if decision.sl is not None:
                    self.pos.sl = decision.sl
                if decision.tp is not None:
                    self.pos.tp = decision.tp
    ...
    def run(self) -> dict:
        ...
        for i in range(self.warmup, len(self.candles)):
            ...
            # Bar hooks (run before decision so the LLM sees post-funding state)
...
# ── Mock decide function for testing without an LLM ──────────────────────────


def _mock_decide(
    symbol: str, candles: list[Candle], position: OpenPosition | None,
) -> LLMDecision:
    ...
    if last > sma:
        return LLMDecision(
            Action.BUY, sl=last - 2 * atr, tp=last + 3 * atr,
            confidence=6, reason="last>sma20", raw_response="",
        )
    return LLMDecision(
        Action.SELL, sl=last + 2 * atr, tp=last - 3 * atr,
        confidence=6, reason="last<sma20", raw_response="",
    )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--interval", default=None,
                   help="Kline interval (default from config)")
...
def main() -> None:
    ...
    logger.info("Backtest %s %s  %s → %s",
                symbol, cfg.kline_interval, start_dt.date(), end_dt.date())

    market = MarketDataClient(testnet=False)
    try:
