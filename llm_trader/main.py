"""
llm_trader — main entry point.

Usage:
    python -m llm_trader                          # uses defaults from .env
    python -m llm_trader --symbol ETHUSDT --leverage 5
    python -m llm_trader --symbol BTCUSDT --leverage 10 --dry-run
    python -m llm_trader --once                   # single evaluation, then exit
"""
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--leverage", type=int, default=None, help="Futures leverage (1-125)")
    p.add_argument("--order-usdt", type=float, default=None, dest="order_usdt",
...
def _detect_inter_cycle_close(
    ...
    # Position was closed (by SL/TP or liquidation) between cycles
    ...
    logger_.info(
        ...
        last_position.side, last_position.entry_price, est_pnl,
    )
...
def _run_cycle(
    ...
    # 1. Fetch candles
    logger_.info("── Fetching %d × %s candles for %s …", cfg.kline_limit, cfg.kline_interval, symbol)
    candles = market.fetch_candles(symbol, cfg.kline_interval, cfg.kline_limit)
    if not candles:
        logger_.error("No candles returned — skipping cycle.")
        return last_position

    current_price = candles[-1].close
    logger_.info("Received %d closed candles (latest close: %s, price: %.2f)",
                 len(candles), candles[-1].timestamp, current_price)
    ...
    logger_.info(
        ...
        position.unrealized_pnl, position.current_sl, position.current_tp,
    )
    ...
    logger_.info(
        ...
        decision.action.value, decision.sl, decision.tp,
    ...
    if decision.action is not Action.HOLD and decision.confidence < cfg.min_confidence:
        ...
        # Still allow HOLD-style SL/TP update if position is open
...
def main() -> None:
    ...
    market = MarketDataClient(testnet=cfg.binance_testnet, base_url=cfg.binance_base_url)
    ...
    try:
        # Apply leverage once on startup
        ...
        while True:
            try:
                last_position = _run_cycle(
                    symbol, cfg, market, engine, trader, tracker, last_position,
                )
